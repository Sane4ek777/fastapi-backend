...
def generate_slug(name, existing_slugs=None, id_suffix=None, cursor=None):
    ...
    name_parts = [re.sub(r'[^a-zA-Z0-9\-]', '', part) for part in name_parts]  # Только латиница и цифры
...
def scrape_product_features(url):
...
def load_xml_to_db():
    ...
    for item in root.findall(".//offer"):
        ...
        cursor.execute("INSERT INTO products VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                       (product_id, name, slug, price, price_rrc, description, images_str, category_id, available, is_diamir))
        attributes = scrape_product_features(url)
        for attr in attributes:
            cursor.execute("INSERT INTO product_attributes (product_id, attribute_name, attribute_value) VALUES (?, ?, ?)",
