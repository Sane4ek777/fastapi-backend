...
const ProductList = ({ selectedCategory, searchQuery }) => {
  ...
  useEffect(() => {
    ...
    loadInitialProducts();
  }, [selectedCategory, searchQuery, sortOrder]); // Перезапуск при изменении фильтров
  ...
  const fetchProducts = async (offset, limit, isInitial = false) => {
    try {
      ...
      if (isInitial) {
        setAllProducts(data.products);  // Теперь извлекаем товары из data.products
      } else {
        setAllProducts((prev) => [...prev, ...data.products]);  // Добавляем новые товары к уже загруженным
      }
    } catch (err) {
  ...
  }, [allProducts, visibleCount, selectedCategory, searchQuery, sortOrder]);  // Учитываем все параметры
